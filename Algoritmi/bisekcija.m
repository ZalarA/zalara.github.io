...
% kjer je f(a)*f(b)<0. Funkcija f podamo v obliki inline funkcije 
...
disp(sprintf('Korak,   Levo krajisce,    Desno Krajisce,   Razpolovisce,        Vrednost'));

while abs(b-a)>epsilon & abs(fc)>epsilon;
   c=a+(b-a)/2;
   korak=korak+1;           				 % nepomembno, stejemo korake za izpis na zaslon
   disp(sprintf('%3d: %15.15f, %15.15f, %15.15f,  %15.15f',[korak,a,b,c,f(c)]));  % nepomembno, izpisemo trenutno sredinsko tocko
   fc=f(c);
   if sign(fa)==sign(fc) 
      a=c;
      fa=fc;
